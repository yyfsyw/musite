 ...
 */
public final class MultiSimMatrixSequenceClosenessCalculator implements ClosenessCalculator, Serializable {
    private static final long serialVersionUID = -82979909064557118L;

    private static class SerializationProxy implements Serializable {
        private static final long serialVersionUID = 4156745873462469438L;

    private final double[][][] matrix;
    private final int windowOffset;
        SerializationProxy(MultiSimMatrixSequenceClosenessCalculator actualObj) {
            this.matrix = actualObj.matrix;
            this.windowOffset = actualObj.windowOffset;
        }

        private Object readResolve() {
            return new MultiSimMatrixSequenceClosenessCalculator(matrix, windowOffset);
        }
    }
     ...
     * @param windowOffset
     */
    public MultiSimMatrixSequenceClosenessCalculator(final int windowOffset) {
        this((double[][][])null, windowOffset);
    }

    /*
    public MultiSimMatrixSequenceClosenessCalculator(final SimilarityMatrix simMatrix,
    ...
    public MultiSimMatrixSequenceClosenessCalculator(final double[][][] simMatrixByAscii,
            final int windowOffset) {
/*        if (simMatrixByAscii!=null && (simMatrixByAscii.length!=128
    ...
    public void setSimilarityMatrix(double[][][] simMatrixByAscii) {
        if (simMatrixByAscii==null)
            this.matrix = null;
        this.matrix = simMatrixByAscii;
    }
    ...
    public double calculate(final Instance instance1,
        ...
        for (int i=0; i<windowOffset; i++) {
            // ignore the center
                return_this += matrix[i][seq1.charAt(i)][seq2.charAt(i)];
                return_this += matrix[windowOffset+i][seq1.charAt(windowOffset+i+1)][seq2.charAt(windowOffset+i+1)];
        }
